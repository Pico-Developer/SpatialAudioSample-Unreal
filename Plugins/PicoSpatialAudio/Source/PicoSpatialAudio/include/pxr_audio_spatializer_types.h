...
typedef struct PxrAudioSpatializer_SourceConfig {
    ...
    float position[3] = {0.f, 0.f, 0.f};                           ///< Position of this source
    float front[3] = {0.f, 0.f, 1.f};                              ///< Front direction of this source
    float up[3] = {0.f, 1.f, 0.f};                                 ///< Up direction of this source
    float radius = 0.1f;                                           ///< Volumetric Radius of this source
    bool use_direct_path_spread = false;  ///< Whether we use explicit source spread parameter, which is useful when
                                          ///< audio middleware specifies spread value by itself.
    float direct_path_spread =
        0.0f;  ///< Explicit source spread, which is useful when audio middleware specifies spread value by itself.
    bool enable_doppler = false;  ///< Whether Doppler effect rendering is on for this source
